...
ExecuteButton.Text = "[ GET MAX REWARD ]" -- Changed Text
...
-- Variable to hold the remote event
local giveRewardEvent = nil

-- Wrap in pcall to prevent script from erroring if paths don't exist
pcall(function()
    -- Wait for the necessary folders and remote event
    local CratesUtilities = ReplicatedStorage:WaitForChild("CratesUtilities", 10)
    local RemotesFolder = CratesUtilities:WaitForChild("Remotes", 10)
    giveRewardEvent = RemotesFolder:WaitForChild("GiveReward", 10)
end)

ExecuteButton.MouseButton1Click:Connect(function()
    -- First, check if the remote event was found
    if not giveRewardEvent then
        StatusLabel.Text = "STATUS: FAILED! (Event not found)"
        StatusLabel.TextColor3 = Color3.fromRGB(255, 0, 85) -- Red for failure
        return -- Stop the function here
    end

    -- Update UI to show it's working
    StatusLabel.Text = "STATUS: SENDING REWARD REQUEST..."
    ...
    -- Define the amount for the reward
    local rewardAmount = 999999999999999999
    
    -- Fire the remote event to the server
    -- Using pcall to prevent other errors (like the event being removed mid-game)
    local success, err = pcall(function()
        giveRewardEvent:FireServer(rewardAmount)
    end)
    
    -- Update UI based on the result
    if success then
        StatusLabel.Text = "STATUS: REWARD SENT!"
        StatusLabel.TextColor3 = Color3.fromRGB(0, 255, 0) -- Green for success
    else
        StatusLabel.Text = "STATUS: FAILED! (Event patched or error)"
        StatusLabel.TextColor3 = Color3.fromRGB(255, 0, 85) -- Red for failure
        warn("Error firing GiveReward event:", err) -- Print error to console for debugging
    end

    -- Reset the button after a delay so the user can see the status
    task.wait(2)
    ExecuteButton.Text = "[ GET MAX REWARD ]"
    ExecuteButton.BackgroundColor3 = Color3.fromRGB(0, 150, 75)
    StatusLabel.Text = "STATUS: IDLE"
    StatusLabel.TextColor3 = Color3.fromRGB(255, 255, 0)
